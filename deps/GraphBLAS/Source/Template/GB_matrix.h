...
int32_t nvec ;          // number of non-empty vectors for hypersparse form,
                        // or same as vdim otherwise.  nvec <= plen.
                        // some of these vectors in Ah may actually be empty.
int32_t nvec_nonempty ; // the actual number of non-empty vectors, or -1 if
                        // not known

int32_t *h ;            // list of non-empty vectors: h_size >= 8*max(plen,1)
int32_t *p ;            // pointers: p_size >= 8*(plen+1)
int32_t *i ;            // indices:  i_size >= 8*max(anz,1)
...
uint32_t nzombies ;     // number of zombies marked for deletion
